...
public class ExecutionAspect {
     ...
     * 定义切面执行的方法
     */
    @Pointcut("execution(* demo.spring.boot.docker.controller.aspect.AspectController.*(..))")
    ...
    public Object aroundAdvice(ProceedingJoinPoint joinPoint) throws Throwable {
        logger.info("第一步【执行Around：拦截方法的签名】joinPoint.getSignature() - > {}", joinPoint.getSignature());//Response FrameworkController.framework(String,String)
        logger.info("第一步【执行Around：拦截目标的对象】joinPoint.getTarget() - > {}", joinPoint.getTarget());//FrameworkController@25c6ab3f
        logger.info("第一步【执行Around：获取链接点的静态部分】joinPoint.getStaticPart() - > {}", joinPoint.getStaticPart());//execution(Response FrameworkController.framework(String,String))
        ...
        logger.info("第一步【执行Around：拦截AOP的当前执行对象】joinPoint.getThis() - > {}", joinPoint.getThis());//FrameworkController@25c6ab3
